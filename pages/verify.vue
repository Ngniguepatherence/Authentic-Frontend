<template>
  <div class="flex w-screen md:flex-row py-10 flex-col px-2  bak h-full min-h-screen justify-center items-center   ">
    ...
    <div class="max-w-[700px] w-full  md:px-8 px-1  -bg-gray-100 ">
      ...
      <div>
        <h1
          class=" uppercase text-4xl text-center font-extrabold leading-none tracking-tight text-gray-900 md:text-4xl mb-6  dark:text-white">
          Verify  
      ...
      <div class="flex items-center justify-center w-full">
        <label for="dropzone-file"
          ...
          <input  @input="handleFileSelect" id="dropzone-file" type="file" class="hidden"
      ...
      <div v-if="fileToUpload.length != 0 || fileUploaded.length != 0"
        ...
        <div
          ...
          <div class="-flex hidden  flex-wrap justify-between items-center gap-x-3">
            <div>
        ...
        <div v-if="fileToUpload.length != 0">
          ...
          <!-- Progress Bar --> 
          <div v-if="isUploaded" class="-flex hidden  items-center gap-x-3 whitespace-nowrap">
  ...
  <div class="mt-2">
    ...
    <div class="card flex justify-content-center">
        ...
        <Dialog  v-model:visible="openResult" maximizable modal header="Authenticity result" :style="{ width: '60rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }  ">
          ...
          <div v-if="loading" class="w-full h-[400px] flex justify-center items-center " >
            <h1 class="text-red-500 text-center font-semibold text-4xl" >Documents no found</h1>

             </div>

          <div v-else>
            <h1 class="text-green-500 text-center font-semibold text-xl" >Trust but verify</h1>
           <iframe class="mt-4"  :src="pdfUrl" width="100%" height="600px"></iframe>
...
console.log(formData);
 try {
    ...
    read.value = true
    const response = await $fetch(run.public.backendUrl + '/api/doc/verifypdf', {
      method: 'post',      
      body:formData
    })
   // pending.value = false
