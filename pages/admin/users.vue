                   ...
                   <div
                       ...
                       <table class="min-w-full  lg:px-8 divide-y divide-gray-200 dark:divide-neutral-700">
                           ...
                           <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                               <tr v-for="(doc,key ) in users.filter(doc => doc.name.includes(store.search)).slice( startAt , startAt + shift) " :key="key">
                                   ...
                                   <td class="size-px whitespace-nowrap">
                                       <div class="ps-6 lg:ps-3 xl:ps-0 pe-6 py-3">
                                           <div class="flex items-center gap-x-3">
                                               <img class="inline-block w-[37px]"
                                                   src="/profile.png"
                                                   alt="Image Description">
                                               <div class="grow">
                                                   <span
                                                       class="block text-sm font-semibold text-gray-800 dark:text-neutral-200">{{ doc.name}} </span>
                                                   <span
                                   ...
                                   <td class="size-px whitespace-nowrap">
                                       <div class="px-6 py-3">
                                           ...
                                           <span class="block text-sm text-gray-500 dark:text-neutral-500">{{ doc.email}}
                                               </span>
                                       </div>
                                   </td>
                                   <td class="size-px whitespace-nowrap">
                                       <div class="px-6 py-3">
                                           <span 
                                               class="py-1 px-1.5 inline-flex items-center gap-x-1 text-xs font-medium bg-blue-200 text-teal-800 rounded-full dark:bg-teal-500/10 dark:text-teal-500  " :class="{'bg-teal-300 text-gray-100':doc.role==='admin'}">
                       ...
                       <div
                           ...
                           <div>
                               <p class="text-sm text-gray-600 dark:text-neutral-400">
                                   <span class="font-semibold ml-1 mr-1 space-x-3 text-gray-800 dark:text-neutral-200">{{ users.length}}   results</span>
...
store.modalDelete.id = doc._id
store.modalDelete.text = 'user ' + doc.name 
store.modalDelete.type = 'User'
store.modal.delete = true
}

const openModalUser = () => {
store.modal.user = true
}
...
const _token = useCookie('_token')
    ...
    const run = useRuntimeConfig()
        try {
            pending.value = true
            const response = await $fetch(run.public.backendUrl + '/api/institutions/user-by-institution', {
                method: 'POST',
                body: {
